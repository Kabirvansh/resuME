...
// ResumeContext.jsx = Single source of truth for resume data in the frontend.
// - initialState defines the default values shown in the UI and preview.
// - useResume() gives you the current state anywhere in the app.
// - useResumeDispatch() lets you update parts of the state via actions.
//
// Data model overview (keep these in sync with the PDF template):
// personalInfo: { name, email, phone, linkedin, github }
// education: [ { university, degree, location, dates, coursework } ]
// experience: [ { title, organization, location, dates, items: string[] } ]
// projects: [ { name, techs, date, items: string[] } ]
// skills: { CategoryName: string | string[] }
// availability: string[]
//
// IMPORTANT: Backend (Streamlit/templates/resume_template.html) uses:
// - experience.organization (not .company)
// - projects.techs (not .tech)
//
// To add new fields: add them to initialState and handle them in the reducer.
// For backend PDFs, also ensure buildResumeContext returns the new fields.
//
// Tailwind note: Context is purely data; styling lives in components like FormPage and LiveResumePreview.
// Changing how data looks does not require changes here.
//
...
const initialState = {
  ...
  skills: {
    Languages:
      "Java, Python, SQL, JavaScript/TypeScript, C, HTML, Flask, React, Next.js, TailwindCSS, Bootstrap",
...
// Reducer function: describes how actions change the state
function resumeReducer(state, action) {
  switch (action.type) {
    ...
    case "UPDATE_EDUCATION":
      return {
        ...state,
        // Expect an array for education; pass a full updated array in payload
